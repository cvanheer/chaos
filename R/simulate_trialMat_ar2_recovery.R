...
#' @param n.subject numerical subject number / repetition of simulation
...
simulate_trialMat_ar2_recovery <- function(n.subject, n.types, n.trials, a1, a2, gen.mean, sigma.ar2, sigma.ar2.bound, n.burnin){
  ...
  # Form into parameter list for the rest
  params <- list(n_subject = n.subject,
                 n_types = n.types,
                 n_trials = n.trials,
                 a1 = a1,
                 a2 = a2,
 ...
 # Simulate AR2 process using each row as input parameters
  trialMat <- trialMat |>
    dplyr:: mutate(
      # Run the function which constrains the sd of the entire resåulting time series even more -
      dataset = purrr::pmap(list(n_trials, a1, a2, gen_mean, sigma.ar2, sigma.ar2.bound, sigma_innov, n_burnin),
